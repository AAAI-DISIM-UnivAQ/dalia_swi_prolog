...
def build(*, src, dali):
    ...
    subprocess.run("pkill -9 swipl 2>/dev/null || true", shell=True)
    ...
    cmds = {"server": f"{swipl} -l {os.path.join(dali_src, 'active_server_wi.pl')} -g go"}
    ...
    for name in src_data["agents"]:
        ...
        setup_path = os.path.join(setupsdir, f'{name}.txt')
        cmds[f"{name}"] = f"""{swipl} -l {os.path.join(dali_src, 'active_dali_wi.pl')} -g "start0('{setup_path}')" """
        
    cmds["user"] = f"{swipl} -l {os.path.join(dali_src, 'active_user_wi.pl')} -g user_interface"
    return cmds, dali_src

class InteractiveShell(ui.element):
    def __init__(self, *, cmd: str, title: str, value: str ='', cwd: str = None, _client = None):
        ...
        with self:
            ...
            self.process = subprocess.Popen(shlex.split(cmd), stdin=self.slave_fd, stdout=self.slave_fd, stderr=self.slave_fd, text=True, bufsize=0, cwd=cwd)
...
class Main(ui.row):
    ...
    async def __call__(self):
        with self:
            ...
            result = await run.cpu_bound(build, src=os.path.abspath(args.src), dali=os.path.abspath(args.dali))
            self.remove(waiting)
        with self.grid:
            if result:
                cmds, dali_src = result
                server = cmds.pop('server')
                InteractiveShell(cmd=server, title='server', cwd=dali_src)
                await asyncio.sleep(5)
                user = cmds.pop('user')
                InteractiveShell(cmd=user, title='user', value="", cwd=dali_src)
                await asyncio.sleep(5)
                for title, cmd in cmds.items():
                    InteractiveShell(cmd=cmd, title=title, cwd=dali_src)
                    await asyncio.sleep(5)
...
if __name__ in {"__main__", "__mp_main__"}:
    ...
    # SWI-Prolog is expected to be in PATH (installed via apt)
